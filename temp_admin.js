        ...
        async function loadMessages(offset = 0, reset = false) {
            if (!currentBotId) {
                document.getElementById('database-messages-list').innerHTML = '<div style="padding: 20px; text-align: center; color: #6b7280;">Select a bot to view messages</div>';
                return;
            }

            try {
                const userId = document.getElementById('messages-user-filter')?.value || '';
                const url = `${API_BASE}/bots/${currentBotId}/messages?skip=${offset}&limit=${MESSAGES_PER_PAGE}${userId ? `&user_id=${userId}` : ''}`;
                const res = await fetch(url);
                const messages = await res.json();
                const container = document.getElementById('database-messages-list');
                ...
                if (messages.length === 0 && reset) {
                    container.innerHTML = '<div style="padding: 20px; text-align: center; color: #6b7280;">No messages found</div>';
                ...
                messages.forEach(msg => {
                    ...
                    const userDisplay = msg.user_username ? `@${msg.user_username}` : (msg.user_name || msg.user_external_id || 'Unknown');
                    const timestamp = msg.timestamp ? new Date(msg.timestamp).toLocaleString('uk-UA', {
                        day: '2-digit',
                        month: '2-digit',
                        year: 'numeric',
                        hour: '2-digit',
                    ...
                    avatarDiv.style.cssText = `
                        ...
                        height: 40px;
                        border-radius: 50%;
                        background: ${isUser ? '#0088cc' : '#e5e5e5'};
                        display: flex;
                        align-items: center;
                    ...
                    bubbleDiv.style.cssText = `
                        max-width: 70%;
                        background: ${isUser ? '#0088cc' : '#ffffff'};
                    ...
                    // Message header (user name + timestamp)
                    const headerDiv = document.createElement('div');
                    headerDiv.style.cssText = `
                        font-size: 12px;
                        font-weight: 600;
                        margin-bottom: 4px;
                        color: ${isUser ? '#ffffff' : '#0088cc'};
                        opacity: 0.9;
                    `;
                    headerDiv.textContent = `${userDisplay} • ${timestamp}`;
                    ...
                    // Format message content (preserve HTML from Telegram)
                    const formattedContent = formatTelegramMessage(msg.content);
                ...
                // Scroll to bottom if reset
                if (reset) {
                    const scrollContainer = document.getElementById('database-messages-container');
                    scrollContainer.scrollTop = scrollContainer.scrollHeight;
                }
            } catch (error) {
                console.error('Error loading messages:', error);
                document.getElementById('database-messages-list').innerHTML = '<div style="padding: 20px; text-align: center; color: #ef4444;">Error loading messages: ' + error.message + '</div>';
            }
        }
        ...
        async function loadUsersForUsersFilter() {
            if (!currentBotId) return;
            ...
            try {
                ...
                const cached = getCached(cacheKey);
                if (cached) {
                    const select = document.getElementById('users-user-filter');
                    if (select) {
                        select.innerHTML = '<option value="">All users</option>' + cached;
                    }
                    return;
        ...
        // messagesOffset already declared at line 1551
        // const MESSAGES_PER_PAGE already declared at line 1553
        ...
        async function loadMessages(offset = 0, reset = false) {
            ...
            try {
                ...
                messages.forEach(msg => {
                    ...
                    // Response timestamp
                    const responseTimestamp = msg.response_timestamp ? new Date(msg.response_timestamp).toLocaleString('uk-UA', {
                        day: '2-digit',
                        month: '2-digit',
                        hour: '2-digit',
                        minute: '2-digit',
                    ...
                    row.innerHTML = `
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px;">${dateTimeStr}</td>
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px;">${msg.user_id.substring(0, 8)}...</td>
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px;">${msg.external_id || '-'}</td>
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px;">${username}</td>
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px; text-align: center;">
                        ...
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px; word-break: break-all; max-width: 100px;">${wallet}</td>
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px; text-align: center;">${totalInvited}</td>
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px; text-align: center;">${topBadge}</td>
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px;">${msg.balance || 0}</td>
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px; text-align: center;">${msg.is_active ? '✅' : '❌'}</td>
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px;">${createdDate}</td>
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px;">${lastActivityDate}</td>
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px; max-width: 150px;">
                        ...
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px; max-width: 200px;">
                            <div onclick="toggleExpand('${responseExpandId}')" style="cursor: pointer; display: flex; align-items: center; gap: 4px;">
                                <span style="font-size: 9px; color: #6b7280;">${responsePreview}</span>
                                <span style="font-size: 8px; color: #9ca3af;">▼</span>
                            </div>
                        ...
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px;">${responseTimestamp}</td>
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px; text-align: center;">${responseTimeCell}</td>
                        <td style="padding: 6px 8px; border-bottom: 1px solid #e5e7eb; font-size: 9px; position: sticky; right: 0; background: white; z-index: 5; white-space: nowrap;">
                            <button onclick="showEditUserForm('${msg.user_id}')" style="background: #059669; color: white; padding: 4px 8px; border: none; border-radius: 3px; cursor: pointer; font-size: 9px; font-weight: 600; margin-right: 4px;">Edit</button>
                            <button onclick="deleteUser('${msg.user_id}', '${msg.external_id || msg.user_id}')" style="background: #dc2626; color: white; padding: 4px 8px; border: none; border-radius: 3px; cursor: pointer; font-size: 9px; font-weight: 600;">Del</button>
                        </td>
                    `;
        ...
        async function toggleUserRow(userId) {
            const messagesRow = document.getElementById(`messages-row-${userId}`);
            const messagesContent = document.getElementById(`messages-content-${userId}`);

            if (!messagesRow) return;
            ...
            if (isVisible) {
                ...
                // Update arrow icon
                const arrow = document.querySelector(`tr[onclick*="toggleUserRow('${userId}')"] td:first-child span`);
                if (arrow) arrow.textContent = '▶';
            } else {
                // Expand - load messages if not loaded yet
                ...
                if (messagesContent.innerHTML.includes('Завантаження') || messagesContent.innerHTML.trim() === '') {
                    await loadUserMessages(userId, messagesContent);
                }
            }
        }

        async function loadUserMessages(userId, container) {
            if (!currentBotId) return;

            try {
                const res = await fetch(`${API_BASE}/bots/${currentBotId}/users/${userId}/messages?limit=50`);
                const messages = await res.json();

                if (messages.length === 0) {
                    container.innerHTML = '<div style="text-align: center; color: #6b7280; font-size: 10px; padding: 20px;">Немає повідомлень</div>';
                    return;
                }
                ...
                messages.forEach(msg => {
                    ...
                    html += `
                        <div style="display: flex; align-items: flex-start; gap: 8px; padding: 6px; background: ${isUser ? '#e0f2fe' : '#f3f4f6'}; border-radius: 4px; border-left: 3px solid ${isUser ? '#0284c7' : '#6b7280'};">
                            <span style="font-size: 12px; flex-shrink: 0;">${isUser ? '👤' : '🤖'}</span>
                            <div style="flex: 1; min-width: 0;">
                                <div style="font-size: 9px; color: #6b7280; margin-bottom: 2px;">${timestamp}</div>
                                <div style="font-size: 10px; color: #374151; word-wrap: break-word; white-space: pre-wrap;">${formatTelegramMessage(msg.content || '')}</div>
                            </div>
                        </div>
                    `;
                });
                ...
                container.innerHTML = `<div style="text-align: center; color: #ef4444; font-size: 10px; padding: 20px;">Помилка завантаження: ${error.message}</div>`;
            }
        }

        async function loadPartnersData(offset = 0, reset = false) {
            if (!currentBotId) return;

            try {
                // Load partners with pagination from API
                const res = await fetch(`${API_BASE}/bots/${currentBotId}/partners?skip=${offset}&limit=${ROWS_PER_PAGE}`);
                const partners = await res.json();
                const tbody = document.getElementById('database-partners-tbody');
                ...
                if (partners.length === 0 && reset) {
                    tbody.innerHTML = '<tr><td colspan="7" style="padding: 20px; text-align: center; color: #6b7280;">No partners found</td></tr>';
                    document.getElementById('database-partners-more').style.display = 'none';
                    return;
                }
                ...
                partners.forEach(partner => {
                    const row = document.createElement('tr');
                    row.innerHTML = `
                        <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; font-size: 11px;">${partner.id.substring(0, 8)}...</td>
                        <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;"><strong>${partner.bot_name}</strong></td>
                        <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;"><span style="background: ${partner.category === 'TOP' ? '#fbbf24' : '#60a5fa'}; color: white; padding: 2px 6px; border-radius: 4px; font-size: 11px;">${partner.category}</span></td>
                        <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; word-break: break-all; max-width: 300px;">
                            <a href="${partner.referral_link || '#'}" target="_blank" style="color: #2563eb; text-decoration: none; font-size: 11px; line-height: 1.4;">
                                ${partner.referral_link || '-'}
                            </a>
                        </td>
                        <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">${partner.active === 'Yes' ? '✅' : '❌'}</td>
                        <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">${partner.commission}%</td>
                        <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; font-size: 11px;">${partner.deleted_at ? new Date(partner.deleted_at).toLocaleDateString() : '-'}</td>
                        <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; font-size: 11px;">-</td>
                    `;
                    tbody.appendChild(row);
                });

                partnersOffset += partners.length;
                // Show "Load more" if we got full page (might be more)
                document.getElementById('database-partners-more').style.display = partners.length === ROWS_PER_PAGE ? 'block' : 'none';
            } catch (error) {
                console.error('Error loading partners:', error);
                document.getElementById('database-partners-tbody').innerHTML = '<tr><td colspan="7" style="padding: 20px; text-align: center; color: #ef4444;">Error loading partners</td></tr>';
            }
        }

        async function loadTranslationsData(offset = 0, reset = false) {
            try {
                const res = await fetch(`${API_BASE}/translations?skip=${offset}&limit=${ROWS_PER_PAGE}`);
                const translations = await res.json();
                const tbody = document.getElementById('database-translations-tbody');
                ...
                if (translations.length === 0 && reset) {
                    tbody.innerHTML = '<tr><td colspan="4" style="padding: 20px; text-align: center; color: #6b7280;">No translations found</td></tr>';
                    document.getElementById('database-translations-more').style.display = 'none';
                    return;
                }

                translations.forEach(trans => {
                    const row = document.createElement('tr');
                    const preview = trans.text.length > 50 ? trans.text.substring(0, 50) + '...' : trans.text;
                    row.innerHTML = `
                        <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;"><code>${trans.key}</code></td>
                        <td style="padding: 10px; border-bottom: 1px solid #e5e7eb;">${trans.lang}</td>
                        <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; font-size: 12px;">${preview}</td>
                        <td style="padding: 10px; border-bottom: 1px solid #e5e7eb; font-size: 11px;">${new Date(trans.created_at).toLocaleDateString()}</td>
                    `;
                    tbody.appendChild(row);
                });

                translationsOffset += translations.length;
                document.getElementById('database-translations-more').style.display = translations.length === ROWS_PER_PAGE ? 'block' : 'none';
            } catch (error) {
                console.error('Error loading translations:', error);
                document.getElementById('database-translations-tbody').innerHTML = '<tr><td colspan="4" style="padding: 20px; text-align: center; color: #ef4444;">Error loading translations</td></tr>';
            }
        }


        function loadMorePartners() {
            loadPartnersData(partnersOffset, false);
        }

        function loadAllPartners() {
            // Reset and load first page
            loadPartnersData(0, true);
        }

        function loadMoreTranslations() {
            loadTranslationsData(translationsOffset, false);
        }

        function loadAllTranslations() {
            loadTranslationsData(0, true);
        }
